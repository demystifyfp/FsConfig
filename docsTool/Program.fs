...
module GenerateDocs =
    ...
    let copyAssets (cfg: Configuration) =
        Shell.copyDir
            (cfg.DocsOutputDirectory.FullName
             </> "content")
    ...
    let regexReplace (cfg: Configuration) source =
        let replacements = [ "{{siteBaseUrl}}", (cfg.SiteBaseUrl.ToString().TrimEnd('/')) ]
    ...
    let generateDocs (docSourcePaths: IGlobbingPattern) (cfg: Configuration) =
        ...
        let format (doc: LiterateDocument) =
            if
                not
                <| Seq.isEmpty doc.Errors
            then
                failwithf
                    "error while formatting file %s. Errors are:\n%A"
                    doc.SourceFile
                    doc.Errors
            else
    ...
    let generateAPI (cfg: Configuration) =
        ...
        let generate (projInfo: string) =
            ...
            let indexDoc = {
                ...
                Content =
                    [
                        Namespaces.generateNamespaceDocs
                            generatorOutput.AssemblyGroup
                            generatorOutput.Properties
                    ]
    ...
    let watchDocs (refreshWebpageEvent: Event<_>) (cfg: Configuration) =
        ...
        let apiDocsWatcher =
            ...
            let glob =
                ...
                |> Seq.map (fun p ->
                    (FileInfo p).Directory.FullName
                    </> "**"
                ) // Create glob for all files in fsproj folder
                |> Seq.fold ((++)) (!! "") // Expand to get all files
                |> Seq.filter (fun file ->
                    typesToWatch
                    |> Seq.exists file.EndsWith
                ) // Filter for only F# style files
...
let main argv =
    try
        ...
        use __ =
            AppDomain.CurrentDomain.ProcessExit.Subscribe(fun _ ->
                dispose tempDocsOutDir
                dispose publishPath
            )

        use __ =
            Console.CancelKeyPress.Subscribe(fun _ ->
                dispose tempDocsOutDir
                dispose publishPath
            )
