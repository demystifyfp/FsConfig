...
module ``Given required environment variables not exist`` =
    ...
    let ``getRecord with custom prefix should return not found error`` () =
        let result = EnvConfig.Get<CustomPrefixSampleConfig>()

        let expected =
            NotFound "MYAPP_PROCESS_ID"
            |> Error
    ...
    let ``getRecord with custom separator should return not found error`` () =
        let result = EnvConfig.Get<CustomSeparatorSampleConfig>()
    ...
    let ``getRecord with custom Config Name Canonicalizer should return not found error`` () =
        let result = EnvConfig.Get<SampleConfig> lowerCaseConfigNameCanonicalizer
...
module ``Given required environment variables exists`` =
    ...
    type ``with valid values``() =
        let expectedRecord: SampleConfig = {
            ProcessId = 123
            ProcessName = "fsconfig.exe"
        }

        let expectedCustomPrefixRecord: CustomPrefixSampleConfig = {
            ProcessId = expectedRecord.ProcessId
        ...
        member __.``getRecord with custom separator should succeeds``() =
            test
                <@ EnvConfig.Get<CustomSeparatorSampleConfig>() = Ok expectedCustomSeparatorRecord @>

        [<Test>]
        member __.``getRecord with custom Config Name Canonicalizer should succeeds``() =
            test
                <@ EnvConfig.Get<SampleConfig> lowerCaseConfigNameCanonicalizer = Ok expectedRecord @>
    ...
    let ``get Enum should still be case-sensitive if it matters`` () =
        ...
        test
            <@ EnvConfig.Get<CaseSensitiveColor> "ENV_ENUM_STRING" = Ok CaseSensitiveColor.PURPLE @>
    ...
    let ``get DU should succeed case-sensitively when it matters`` () =
        ...
        test
            <@
                EnvConfig.Get<CaseSensitiveDuConfig>() = Ok
                                                             {
                                                                 CaseSensitiveDuColor =
                                                                     CaseSensitiveDuColor.Purple
                                                             }
            @>
...
module ``Getting option type`` =
    ...
    type StringOptionConfig = { StringOption: string option }
    type EnumOptionConfig = { ColorOption: Color option }
...
module ``Getting record with mutlitple option type fields`` =
    ...
    let ``return record with corresponding option value`` () =
        ...
        test
            <@
                EnvConfig.Get<Config>() = Ok(
                    {
                        ProcessCount = Some 42
                        Timeout = None
                        AwsAccessKeyId = Some "ID-123"
                    }
                )
            @>
...
module ``Getting list type`` =
    ...
    type IntListConfig = { IntList: int list }
    ...
    let ``return list if environment variable exist with mutiple comma separated values`` () =
        ...
        test
            <@
                EnvConfig.Get<IntListConfig>() = Ok
                                                     {
                                                         IntList =
                                                             [
                                                                 42
    ...
    let ``return list if environment variable exist with mutiple | separated values`` () =
        ...
        test
            <@
                EnvConfig.Get<IntListUsingPipesConfig>() = Ok
                                                               {
                                                                   IntListUp =
                                                                       [
                                                                           42
